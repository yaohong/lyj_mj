...
-define(DEFAULT_FORMAT, "[%L] [%E] [%Y-%M-%D %h:%m:%s] %l%n").
...
-record(file_appender, {dir, file_name, fd, counter, log_type, rotation, suffix, level, format=?DEFAULT_FORMAT, his_file_name = []}).
...
%% log record
-record(log, {level, env :: {Pid :: pid(), FileName :: string(), Line :: integer()}, msg, data, time, millis}).