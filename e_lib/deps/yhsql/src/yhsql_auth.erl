...
%%%           in the file yhsql.erl - I just moved it here.
%%%
...
%%--------------------------------------------------------------------
%% External exports (should only be used by the 'yhsql_conn' module)
%%--------------------------------------------------------------------
-export([
    do_old_auth/6,
    do_new_auth/7
...
%% Returns : result of yhsql_conn:do_recv/3
%%--------------------------------------------------------------------
do_old_auth(Sock, SeqNum, User, Password, Salt1, LogFun) ->
    Auth = password_old(Password, Salt1),
    Packet2 = make_auth(User, Auth),
    yhsql_socket:do_send(Sock, Packet2, SeqNum, LogFun),
    yhsql_socket:do_recv(LogFun, Sock, SeqNum).
...
%% Returns : result of yhsql_conn:do_recv/3
%%--------------------------------------------------------------------
do_new_auth(Sock, SeqNum, User, Password, Salt1, Salt2, LogFun) ->
    Auth = password_new(Password, Salt1 ++ Salt2),
    Packet2 = make_new_auth(User, Auth, none),
    yhsql_socket:do_send(Sock, Packet2, SeqNum, LogFun),
    {success, {Packet3, SeqNum2}} =  yhsql_socket:do_recv(LogFun, Sock, SeqNum),
    case Packet3 of
        <<254:8>> ->
            AuthOld = password_old(Password, Salt1),
            yhsql_socket:do_send(Sock, <<AuthOld/binary, 0:8>>, SeqNum2 + 1, LogFun),
            yhsql_socket:do_recv(LogFun, Sock, SeqNum2 + 1);
        _ ->
            {success, {Packet3, SeqNum2}}
    end.
...
%% part of do_old_auth/4, which is part of yhsql_init/4
...
%% part of do_new_auth/4, which is part of yhsql_init/4
