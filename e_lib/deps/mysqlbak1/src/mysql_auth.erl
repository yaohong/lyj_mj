...
do_old_auth(Sock, SeqNum, User, Password, Salt1, LogFun) ->
    ...
    mysql_socket:do_send(Sock, Packet2, SeqNum, LogFun),
    mysql_socket:do_recv(LogFun, Sock, SeqNum).
...
do_new_auth(Sock, SeqNum, User, Password, Salt1, Salt2, LogFun) ->
    ...
    mysql_socket:do_send(Sock, Packet2, SeqNum, LogFun),
    {success, {Packet3, SeqNum2}} =  mysql_socket:do_recv(LogFun, Sock, SeqNum),
    case Packet3 of
        <<254:8>> ->
            AuthOld = password_old(Password, Salt1),
            mysql_socket:do_send(Sock, <<AuthOld/binary, 0:8>>, SeqNum2 + 1, LogFun),
            mysql_socket:do_recv(LogFun, Sock, SeqNum2 + 1);
        _ ->
