...
decode_qp_packet(?CMD_QP_LOGIN_REQ, Body) ->
    common_pb:decode_qp_login_req(Body);
decode_qp_packet(?CMD_QP_CREATE_ROOM_REQ, Body) ->
    common_pb:decode_qp_create_room_req(Body);
decode_qp_packet(?CMD_QP_JOIN_ROOM_REQ, Body) ->
    common_pb:decode_qp_join_room_req(Body);
decode_qp_packet(?CMD_QP_READY_REQ, Body) ->
    common_pb:decode_qp_ready_req(Body);
decode_qp_packet(?CMD_QP_EXIT_ROOM_REQ, Body) ->
    common_pb:decode_qp_exit_room_req(Body);
decode_qp_packet(?CMD_QP_GAME_DATA, Body) ->
    common_pb:decode_qp_game_data(Body);
decode_qp_packet(?CMD_QP_PING_REQ, Body) ->
    common_pb:decode_qp_ping_req(Body).
...
encode_qp_packet(Packet) when is_record(Packet, qp_ready_rsp) ->
    encode_qp_packet(?CMD_QP_READY_RSP, common_pb:encode_qp_ready_rsp(Packet));
encode_qp_packet(Packet) when is_record(Packet, qp_ready_push) ->
    encode_qp_packet(?CMD_QP_READY_PUSH, common_pb:encode_qp_ready_push(Packet));
...
encode_qp_packet(Packet) when is_record(Packet, qp_room_dismiss_push) ->
    encode_qp_packet(?CMD_QP_ROOM_DISSMISS_PUSH, common_pb:encode_qp_room_dismiss_push(Packet));
