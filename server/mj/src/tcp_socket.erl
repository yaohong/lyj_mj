...
-spec start(
        ...
        UserSup :: module(),
        ReceiveSup :: module(),
		MaxStanzaSize :: integer()
) -> ok.
start(Socket, UserHandle, PacketHeadLen, UserSup, ReceiveSup, MaxStanzaSize) ->
    {ok, ReceiverPid} = tcp_receiver:start(UserHandle, ReceiveSup, PacketHeadLen, Socket, MaxStanzaSize),
    ...
    case UserHandle:start(UserSup, ?MODULE, SocketData) of
        ...
        {error, Reason} ->
			...
			tcp_receiver:close(ReceiverPid),
