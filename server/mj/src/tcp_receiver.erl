...
start(UserHandle, ReceiveSup, PacketHeadLen, Socket) ->
    start(UserHandle, ReceiveSup, PacketHeadLen, Socket, 1024).
...
process_packet(
	...
	try
		case decode_message(Data, PacketHeadLen, MaxPacketSize, UserModule) of
			{continue, DataRes} ->
		...
		Type:What ->
			?FILE_LOG_ERROR("decode_message exception Type=~p What=~p stack=~p", [Type, What, erlang:get_stacktrace()]),
			{error, message_parse_error}
	end.

decode_message(Data, PacketHeadLen, _MaxPacketSize, _UserModule) when size(Data) < PacketHeadLen ->
	{continue, Data};
decode_message(Data, PacketHeadLen, MaxPacketSize, UserModule) ->
%%	SizeLen = PacketHeadLen * 8,
	<<HeadBin:PacketHeadLen/binary, DataRest/binary>> = Data,
	Len = UserModule:head_len(HeadBin, PacketHeadLen),
%% 	<<Len:SizeLen/unsigned-integer-little, DataRest/binary>> = Data,
	if
		Len > MaxPacketSize ->
			?FILE_LOG_ERROR("packet size error [~p]", [Len]),
			throw("packet size exception");
		true -> ok
	end,
