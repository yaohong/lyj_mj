...
init({Dir, _Fname, {Type, Max}, Rot, Suf, Level, Pattern} = _Conf) ->
    ?LOG2("file_appender:init() - 1 ~p~n",[_Conf]),
	%%取当前的时间戳为名字
	OldLogPath = os:getenv("OLD_LOG_PATH"),
	TimeName = get_localtime_str(),
	os:cmd("mv " ++ Dir ++ "/sasl.log" ++ " " ++ OldLogPath ++ "/sasl_" ++ TimeName ++ ".log"),
	os:cmd("mv " ++ Dir ++ "/*." ++ Suf ++ " " ++ OldLogPath),

    File = Dir ++ "/" ++ TimeName ++ "." ++ Suf,
    ...
    State = #file_appender{dir = Dir, file_name = TimeName, fd = Fd, counter=0,
...
rotate(#file_appender{fd = Fd, dir=Dir,  file_name=Fn, counter=Cntr, rotation=Rot, suffix=Suf, log_type=Ltype, level=Level, format=Format, his_file_name = HisFileNameList} = _S) ->
    file:close(Fd),
    ?LOG("Starting rotation~n"),
	CurTime = get_localtime_str(),
	OldFile = Dir ++ "/" ++  Fn ++ "." ++ Suf,
	NewFile = Dir ++ "/" ++  Fn ++ "__" ++ CurTime ++ "." ++ Suf,
	file:rename(OldFile, NewFile),
	NewHisFileNameList =
	if
		length(HisFileNameList) =:= Rot ->
			[RemoveFileName|LT] = lists:reverse(HisFileNameList),
			OldLogPath = os:getenv("OLD_LOG_PATH"),
			os:cmd("mv " ++ RemoveFileName ++ " " ++ OldLogPath),
			[NewFile|lists:reverse(LT)];
		true -> [NewFile|HisFileNameList]
	end,
%%    rotate_file(Dir ++ "/" ++ Fn, Rot - 1, Suf),
    Src = Dir ++ "/" ++ CurTime ++ "." ++ Suf,
    {ok ,Fd2} = file:open(Src, ?FILE_OPTIONS_ROTATE),
    State2 = #file_appender{dir = Dir, file_name = CurTime, fd = Fd2, counter=Cntr, log_type = Ltype, rotation = Rot, suffix=Suf, level=Level, format=Format, his_file_name = NewHisFileNameList},
    {ok, State2}.
...
get_localtime_str() ->
	...
	lists:flatten(io_lib:format("~p-~p-~p_~p:~p:~p", [Y, M, D, Hour, Min, Sec])).