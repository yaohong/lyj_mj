...
do_log(#log{level = L} = Log, #smtp_appender{level=Level} = State) ->
	...
	true ->
	    ...
	    ?LOG2("send mail from=~p, to=~p, m=~p",[From, To, M]),
	    case init_smtp(Ip, Port, Uname, Pass) of 
	    	{success, Pid} ->
...
init_smtp(Ip, Port, Uname, Password) ->
	...
	{undefined, undefined} ->
	    {success, Pid};
	_ ->
	    case catch smtp_fsm:login(Pid, Uname, Password) of 
	    	{ok, Rsp} ->
	    		?LOG2("smtp login success rsp=~p", [Rsp]),
	    		{success, Pid};
	    	Other ->
	    		?LOG2("smtp login fail other=~p", [Other]),
	    		catch Pid ! exit,
	    		fail
	    end
    end.
