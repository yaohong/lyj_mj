...
smtp_start({helo, Name}, _Pid, Info)->
    ...
    {Code, Resp} ->
        {stop, normal, {helo_error, {Code,Resp}}, Info};
    Error ->
        {stop, normal, {conn_error, Error}, Info}
    end;
smtp_start({ehlo, Name}, _Pid, Info) ->
    ?LOG2("ehlo name=~p", [Name]),
    Msg = ["ehlo ", Name, "\r\n"],
    ok = gen_tcp:send(Info#info.socket, Msg),
    case get_response(Info#info.socket) of
    {"250", Resp} ->
        ?LOG2("ehlo success resp=~p", [Resp]),
    ...
    {Code, Resp} ->
        {stop, normal, {ehlo_error, {Code, Resp}}, Info};
    Error ->
        {stop, normal, {conn_error, Error}, Info}
    end.
...
smtp_conn(noop, _Pid, Info) ->
    ...
    {"250", Resp} ->
        {reply, {ok, Resp}, smtp_conn, Info};
    {Code, Resp} ->
        {stop, normal, {noop_error, {Code, Resp}}, Info};
    Error ->
        {stop, normal, {conn_error, Error}, Info}
    end;
smtp_conn({mfrom, Address}, _Pid, Info) ->
    ?LOG2("address=~p", [Address]),
    ...
    {"250", Resp} ->
        {reply, {ok, Resp}, smtp_conn, Info};
    {Code, Resp} ->
        {stop, normal, {mfrom_error, {Code, Resp}}, Info};
    Error ->
        {stop, normal, {conn_error, Error}, Info}
    end;
smtp_conn({rcpt_to, Address}, _Pid, Info) ->
    ...
    {"250", Resp} ->
        {reply, {ok, Resp}, smtp_conn, Info};
    {Code, Resp} ->
        {stop, normal, {rcpt_error, {Code, Resp}},  Info};
    Error ->
        {stop, normal, {conn_error, Error}, Info}
    end;
smtp_conn(data, _Pid, Info) ->
    ...
    {Code, Resp} ->
        {stop, normal, {data_error, {Code, Resp}}, Info};
    Resp ->
        {stop, normal, {conn_error, Resp}, Info}
    end;
smtp_conn({plain_login,User,Pwd}, _Pid, Info) ->
    ?LOG2("plain_login user=~p pwd=~p", [User, Pwd]),
    Msg =  ["AUTH PLAIN ", plain_encode(User,Pwd), "\r\n"],
    ok = gen_tcp:send(Info#info.socket, Msg),
    case get_response(Info#info.socket) of
    {"235", Resp} ->
        ?LOG2("plain_login success resp=~p", [Resp]),
        {reply, {ok, Resp}, smtp_conn, Info};
    {Code, Resp} ->
        ?LOG2("plain_login fail code=~p, resp=~p", [Code, Resp]),
        {stop, normal, {auth_error, {Code, Resp}}, Info};
    Error ->
        ?LOG2("plain_login fail error=~p", [Error]),
        {stop, normal, {conn_error, Error}, Info}
    end;
smtp_conn({login_login,User}, _Pid, Info) ->
    ?LOG2("login_login user=~p", [User]),
    B64Usr =  base64:encode(User),
    Msg = ["AUTH LOGIN ", B64Usr, "\r\n"],
    ok = gen_tcp:send(Info#info.socket, Msg),
    case get_response(Info#info.socket) of
    {"334", Resp} ->
        ?LOG2("login_login success resp=~p", [Resp]),
...
smtp_conn(features, _Pid, Info) ->
    ?LOG2("features = ~p", [Info#info.features]),
...
handle_sync_event(rset, _Pid, _State, Info) ->
    ...
    {"250", Resp} ->
        {reply, {ok, Resp}, smtp_conn, Info};
    {Code, Resp} ->
        {stop, normal, {rset_error, {Code, Resp}}, Info};
    Error ->
        {stop, normal, {conn_error, Error}, []}
    end.
...
terminate(Reason,_StateName,_StateData) ->
    ?LOG2("smtp terminate reason=~p", [Reason]),
    {terminated, Reason}.
