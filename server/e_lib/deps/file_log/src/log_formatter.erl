...
get_token_value(log, Log) ->
	...
	try
		...
		_:_ ->
			io_lib:format("log format error msg[~p] data[~p]", [Msg, Data])
	end;
...
get_token_value(time_zone, Log) ->
    ...
    TZ;
get_token_value(env, Log) ->
	{Pid, ModName, Line} = Log#log.env,
	pid_to_list(Pid) ++ ":" ++ atom_to_list(ModName) ++ ":" ++ integer_to_list(Line);
