...
%% 启动一个MySQL client gen_server process (挂在监控树下)
-spec start(
	Id :: term(),
	Host :: string(),
	Port :: integer(),
	...
	{ok, pid()} |
	{error, _}.
start(Id, Host, Port, User, Password, Database, LogFun) when is_list(Host),
	is_integer(Port),
	is_list(User),
	is_list(Password),
	is_list(Database) ->
	ChildSpec = {?SERVER,
		{?MODULE, start_link, [Id, Host, Port, User, Password, Database, LogFun]},
		transient,
		brutal_kill,
		worker,
		[?MODULE]},
