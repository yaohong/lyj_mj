...
-export([select_query/3,select_query_nowhere/3,
         insert_query/3,
...
yhsql_to_odbc({updated, MySQLRes}) when is_record(MySQLRes, yhsql_result) ->
    {updated, yhsql_result:get_result_affected_rows(MySQLRes)};
yhsql_to_odbc({data, MySQLRes}) when is_record(MySQLRes, yhsql_result) ->
    yhsql_item_to_odbc(
	    yhsql_result:get_result_field_info(MySQLRes),
	    yhsql_result:get_result_rows(MySQLRes));
yhsql_to_odbc({error, MySQLRes}) ->
	yhsql_result:get_result_reason(MySQLRes).

yhsql_item_to_odbc(Columns, Recs) ->
    {selected,
     [element(2, Column) || Column <- Columns],
     [list_to_tuple(Rec) || Rec <- Recs]}.

%% 例子:
%% > Query1 = yhsql_util:select_query("tbl_user", ["username", "password"], "").
%% ["SELECT ","username, password"," FROM ","tbl_user",";"]
%% > iolist_to_binary(Query1).
%% <<"SELECT username, password FROM tbl_user;">>
%%
%% > Query2 = yhsql_util:select_query("tbl_user", ["username", "password"], "username = 'user1'").
%% ["SELECT ","username, password"," FROM ","tbl_user",
...
select_query_nowhere(Table, Fields, Other) ->
    ["SELECT ", string:join([to_list(TI) || TI <- Fields], ", "),
     " FROM ", Table, Other, ";"].

%% 例子:
%% > Query = yhsql_util:insert_query("tbl_user", ["username", "password"], ["user1", "pwd"]).
...
%% 例子:
%% > Query = yhsql_util:replace_query("tbl_user", ["username", "password"], ["user1", "pwd"]).
...
%% 例子:
%% > Query = yhsql_util:insert_ignore_query("tbl_user", ["username", "password"], ["user1", "pwd"]).
...
%% 例子:
%% > Query1 = yhsql_util:delete_query("tbl_user", "").
%% ["DELETE FROM ","tbl_user",";"]
%% > iolist_to_binary(Query1).
%% <<"DELETE FROM tbl_user;">>
%%
%% > Query2 = yhsql_util:delete_query("tbl_user", "username = 'user1'").
...
%% 例子:
%% > Query = yhsql_util:update_query("tbl_user", [{"password", "newpwd"}], "username = 'user1'").
